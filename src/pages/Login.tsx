...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err?.code === 'auth/invalid-credential' ? 'Email or password is incorrect.' : 'Could not sign in. Check your Firebase account configuration and try again.');
    } finally {
      setLoading(false);
    }
  };
        ...
        <TiltCard>
          <Card className="p-7 glass-strong border border-border-strong rounded-2xl shadow-2xl">
            ...
            <div className="mb-4 rounded-xl border border-border bg-white/5 p-3 text-center text-[11px] text-foreground/60">
              Use the email and password for your registered church account. Admin access is assigned by the church owner; it cannot be switched from the browser.
            </div>
